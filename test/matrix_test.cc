...
void ConvertVecToRadian(Eigen::Vector3d &a)
{
  for(int i = 0; i < 3; i ++)
  {
    a[i] = sonia_common::DegreeToRadian(a[i]);
  }
}
...
TEST(MatrixTest, QuatToEuler) {
  ...
  for(int i = 0; i < 2; i++ )
  {
    ...
    test_quaternion = sonia_common::EulerToQuat(euler_ref);
    CompareQuaternion(quat_ref, test_quaternion);

    test_rot = sonia_common::EulerToRot(euler_ref);
    CompareRotationMatrix(rot_ref, test_rot);

    test_euler = sonia_common::QuatToEuler(quat_ref);
    CompareEuler(test_euler, euler_ref);

    test_rot = sonia_common::QuatToRot(quat_ref);
    CompareRotationMatrix(rot_ref, test_rot);

    test_quaternion = sonia_common::RotToQuat(rot_ref);
    CompareQuaternion(test_quaternion, quat_ref);

    test_euler = sonia_common::RotToEuler(rot_ref);
    CompareEuler(test_euler, euler_ref);
  }

  Eigen::Vector3d testVec(2,20, 90);
  ConvertVecToRadian(testVec);
  Eigen::Quaterniond q = sonia_common::EulerToQuat(testVec);
  std::cout << q.w() << "\t"
      << q.x() << "\t"
      << q.y() << "\t"
      << q.z()<< std::endl;
  Eigen::Vector3d v = sonia_common::QuatToEuler(q);
  std::cout << v.x()
      << "\t" << v.y()
      << "\t" << v.z()<< std::endl;
