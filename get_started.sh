...
DEFINE_string 'type' 'robot' 'The type of project to be created (e.g. robot or perception)' 't'
...
DEFINE_string 'dep' 'sonia_msgs' 'Give extra dependencies separated by colons (e.g. sonia_msgs,turtlebot3,qt_dotgraph)' 'd'
...
clean_workflows() {
    if [ $FLAGS_gpu == true ]; then
        if [ $FLAGS_type == "robot" ]; then
            rm .github/workflows/docker-image-perception*
            rm .github/workflows/docker-image-robot-feature.yml
            rm .github/workflows/docker-image-robot-develop.yml
            rm .github/workflows/docker-image-robot-master.yml
        elif [ $FLAGS_type == "perception" ]; then
            rm .github/workflows/docker-image-robot*
            ...
            echo "Provide either perception or robot"
            echo ""
            flags_help
            exit 1
        fi
        ...
        if [ $FLAGS_type == "robot" ]; then
            rm .github/workflows/docker-image-perception*
            rm .github/workflows/docker-image-robot-l4t*
        elif [ $FLAGS_type == "perception" ]; then
            rm .github/workflows/docker-image-robot*
